...
public abstract class VoronoiGraph {
    ...
    abstract public BiomeEnum getBiome(Center p);

    abstract public Color getColor(BiomeEnum biome);
    ...
    public ArrayList<Shape> createMap() {
        return paint(true, true, false, false, false, true);
    }

    public Polygon[] drawPolygon(Center center, Color color) {
        ...
        ArrayList<Integer> x0 = new ArrayList<>(center.neighbors.size() * 3);
        ArrayList<Integer> y0 = new ArrayList<>(center.neighbors.size() * 3);

        for (Center n : center.neighbors) {
            Edge e = edgeWithCenters(center, n);
            ...
            x0.add((int) e.v0.loc.x);
            y0.add((int) e.v0.loc.y);
            x0.add((int) e.v1.loc.x);
            y0.add((int) e.v1.loc.y);
            x0.add((int) center.loc.x);
            y0.add((int) center.loc.y);
            ...
            center.area += Math.abs(center.loc.x * (e.v0.loc.y - e.v1.loc.y)
                    + e.v0.loc.x * (e.v1.loc.y - center.loc.y)
                    + e.v1.loc.x * (center.loc.y - e.v0.loc.y)) / 2;
        }

        double[] tmp0 = new double[x0.size() * 2];
        ...
        Polygon polygon = new Polygon(tmp0);
        ...
        if (edgeCorner2 != null) {
            ...
            if (closeEnough(edgeCorner1.loc.x, edgeCorner2.loc.x, 1)) {
                triangle = new Polygon(
                        center.loc.x, center.loc.y,
                        edgeCorner1.loc.x, edgeCorner1.loc.y,
                        edgeCorner2.loc.x, edgeCorner2.loc.y
                );
            } else {
                triangle = new Polygon(
                        center.loc.x, center.loc.y,
                        edgeCorner1.loc.x, edgeCorner1.loc.y,
                        ((closeEnough(edgeCorner1.loc.x, bounds.x, 1) || closeEnough(edgeCorner2.loc.x, bounds.x, .5)) ? bounds.x : bounds.right), ((closeEnough(edgeCorner1.loc.y, bounds.y, 1) || closeEnough(edgeCorner2.loc.y, bounds.y, .5)) ? bounds.y : bounds.bottom),
                        edgeCorner2.loc.x, edgeCorner2.loc.y
                );
                center.area += 0; //TODO: area of polygon given vertices
            }

            return new Polygon[]{polygon, triangle};
        }

        return new Polygon[]{polygon};
    }

    public Polygon[] createPolygons(Center center) {
        //only used if Center c is on the edge of the graph. allows for completely filling in the outer polygons
        Corner edgeCorner1 = null;
        Corner edgeCorner2 = null;
        center.area = 0;
    ...
    public ArrayList<Shape> paint(boolean drawBiomes, boolean drawRivers, boolean drawSites, boolean drawCorners, boolean drawDelaunay, boolean drawVoronoi) {
        ...
        if (!drawBiomes) {
            ...
            for (int i = 0; i < defaultColors.length; i++) {
                defaultColors[i] = Color.rgb(r.nextInt(255), r.nextInt(255), r.nextInt(255));
            }
        }

        ArrayList<Shape> polygons = new ArrayList<>(centers.size());

        //draw via triangles
        for (Center c : centers) {
            for (Polygon polygon : drawPolygon(c, drawBiomes ? getColor(c.biome) : defaultColors[c.index])) {
                polygons.add(polygon);

                polygon.addEventHandler(MouseEvent.MOUSE_CLICKED, event -> {
                    System.out.println("center index: " + c.index);
                });
            }

            drawPolygon(c, Color.web(String.valueOf(c.index)));
        }
...
//            }
            if (drawRivers && e.river > 0) {
                Line line = new Line(e.v0.loc.x, e.v0.loc.y, e.v1.loc.x, e.v1.loc.y);
                line.setFill(javafx.scene.paint.Color.BLUE);
                line.prefWidth(1 + (int) Math.sqrt(e.river * 2));
